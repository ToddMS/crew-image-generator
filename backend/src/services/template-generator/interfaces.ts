...
export interface TemplateComponent {
  draw(ctx: NodeCanvasRenderingContext2D, data: any, config: any): void;
}

export interface BackgroundComponent {
  draw(ctx: NodeCanvasRenderingContext2D, dimensions: CanvasDimensions, colors: ColorScheme): void;
}

export interface NameDisplayComponent {
  draw(ctx: NodeCanvasRenderingContext2D, crew: Crew, positioning: NamePositioning, colors: ColorScheme): void;
}

export interface BoatStyleComponent {
  draw(ctx: NodeCanvasRenderingContext2D, crew: Crew, positioning: BoatPositioning, colors: ColorScheme): void;
}

export interface TextLayoutComponent {
  draw(ctx: NodeCanvasRenderingContext2D, crew: Crew, positioning: TextPositioning, colors: ColorScheme): void;
}

export interface LogoComponent {
  draw(ctx: NodeCanvasRenderingContext2D, logoData: ClubIconData, positioning: LogoPositioning): void;
}
