...
const GeneratePage: React.FC = () => {
  ...
  useEffect(() => {
    console.log('🔄 Generate page useEffect triggered');
    console.log('📍 location.state:', location.state);
    console.log('👥 crews.length:', crews.length);
    console.log('🚣 crews:', crews.map(c => ({ id: c.id, name: c.boatName })));
    ...
    if (state?.selectedTemplate) {
      console.log('🎨 Setting selected template:', state.selectedTemplate.name);
      setSelectedTemplate(state.selectedTemplate);
    }

    if (state?.selectedCrewIds && state.selectedCrewIds.length > 0 && crews.length > 0) {
      console.log('✅ Found selectedCrewIds:', state.selectedCrewIds);
      
      // Set multiple selected crews
      console.log('🎯 Setting selectedCrews to:', state.selectedCrewIds);
      ...
      console.log('🎯 Looking for crew with ID:', crewId);
      const crew = crews.find((c) => c.id === crewId);
      if (crew) {
        console.log('✅ Found crew:', crew.boatName, 'Setting as selectedCrew');
        setSelectedCrew(crew);
      } else {
        console.log('❌ Could not find crew with ID:', crewId);
      }
    } else {
      console.log('⏭️ Skipping crew selection:', {
        hasSelectedCrewIds: !!state?.selectedCrewIds,
        selectedCrewIdsLength: state?.selectedCrewIds?.length,
        crewsLength: crews.length
      });
    }
  ...
  const loadCrews = async () => {
    ...
    try {
      ...
      if (result.data) {
        ...
        console.log('✅ Crews loaded successfully:', transformedCrews.map(c => ({ id: c.id, name: c.boatName })));
              ...
              {crews.length === 0 ? (
                ...
                crews.map((crew) => (
                  <div
                    ...
                    onClick={() => {
                      console.log('🖱️ Clicked on crew:', crew.boatName, 'ID:', crew.id);
                      const newSelected = new Set(selectedCrews);
                      console.log('📋 Current selectedCrews:', Array.from(selectedCrews));
                      
                      if (newSelected.has(crew.id)) {
                        console.log('➖ Removing crew from selection');
                        newSelected.delete(crew.id);
                      } else {
                        console.log('➕ Adding crew to selection');
                        newSelected.add(crew.id);
                      }
                      
                      console.log('📋 New selectedCrews will be:', Array.from(newSelected));
                      setSelectedCrews(newSelected);
                      
                      // Also update the primary selected crew
                      if (!selectedCrew || selectedCrew.id !== crew.id) {
                        console.log('🎯 Setting crew as primary selectedCrew:', crew.boatName);
                        setSelectedCrew(crew);
                      }
                    }}
                  >
